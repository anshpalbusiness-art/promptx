...
final class ContactsService: ContactsServicing {
    ...
    func search(params: PromptXContactsSearchParams) async throws -> PromptXContactsSearchPayload {
        ...
        return PromptXContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: PromptXContactsAddParams) async throws -> PromptXContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return PromptXContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return PromptXContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> PromptXContactPayload {
        PromptXContactPayload(
