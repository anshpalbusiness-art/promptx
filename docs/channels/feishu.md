...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects PromptX to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
promptx plugins install @promptx/feishu
```

Local checkout (when running from a git repo):

```bash
promptx plugins install ./extensions/feishu
```
...
If you just installed PromptX, run the wizard:
...
2. Configuring app credentials in PromptX
...
1. You already ran `promptx channels add` for Feishu
2. The gateway is running (`promptx gateway status`)
...
```bash
promptx pairing approve feishu <CODE>
```
...
2. Run `promptx logs --follow` and look for `chat_id`
...
2. Run `promptx logs --follow` and look for `open_id`
...
| Command                   | Description                   |
| ------------------------- | ----------------------------- |
| `promptx gateway status`  | Show gateway status           |
| `promptx gateway install` | Install/start gateway service |
| `promptx gateway stop`    | Stop gateway service          |
| `promptx gateway restart` | Restart gateway service       |
| `promptx logs --follow`   | Tail gateway logs             |
...
4. Check logs: `promptx logs --follow`
...
5. Ensure the gateway is running: `promptx gateway status`
6. Check logs: `promptx logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.promptx/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.promptx/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
