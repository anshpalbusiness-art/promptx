...
import type { PromptXConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: PromptXConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("promptx security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("promptx configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("promptx config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("promptx doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("promptx config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
