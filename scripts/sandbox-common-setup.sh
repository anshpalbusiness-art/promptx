...
BASE_IMAGE="${BASE_IMAGE:-promptx-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-promptx-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=promptx.sandbox=1)
NOTE
