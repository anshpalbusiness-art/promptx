...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in PromptX
...
Running PromptX via Node with `tsx` fails at startup with:

```
[promptx] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `promptx status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch promptx.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node promptx.mjs status` works on Node 25.
