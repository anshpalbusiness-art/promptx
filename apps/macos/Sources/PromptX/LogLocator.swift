...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["PROMPTX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/promptx")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("promptx-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("promptx-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/promptx/ (rolling or stdout), or nil if none exist.
