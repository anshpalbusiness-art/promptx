...
This document describes how the PromptX ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`promptx acp` exposes an ACP agent over stdio and forwards prompts to a running
PromptX Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a PromptX Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `promptx acp` over stdio.

Example config:

```bash
promptx config set gateway.remote.url wss://gateway-host:18789
promptx config set gateway.remote.token <token>
```

Example run:

```bash
promptx acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
promptx acp --session agent:main:main
promptx acp --session agent:design:main
promptx acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “PromptX ACP” to start a thread.

## Execution Model

- ACP client spawns `promptx acp` and speaks ACP messages over stdio.
...
```bash
promptx acp --session agent:main:main
promptx acp --session-label "support inbox"
promptx acp --reset-session
```
...
`promptx acp` resolves the Gateway URL and auth from CLI flags or config:
