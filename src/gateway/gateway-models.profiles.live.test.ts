...
import type { PromptXConfig, ModelProviderConfig } from "../config/types.js";
import { resolvePromptXAgentDir } from "../agents/agent-paths.js";
...
import { ensurePromptXModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.PROMPTX_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.PROMPTX_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.PROMPTX_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<PromptXConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.PROMPTX_AGENT_DIR,
  ...
  const hostAgentDir = resolvePromptXAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-live-state-"));
  ...
  process.env.PROMPTX_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.promptx-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolvePromptXAgentDir();
  const sanitizedCfg: PromptXConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "PromptX live tool probe (local, safe): " +
    ...
    process.env.PROMPTX_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.PROMPTX_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensurePromptXModelsJson(cfg);
      ...
      const rawModels = process.env.PROMPTX_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.promptx-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
