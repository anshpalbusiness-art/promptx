/**
 * Twitch channel plugin for PromptX.
 *
...
import { buildChannelConfigSchema } from "promptx/plugin-sdk";
import type {
 ...
 * for PromptX. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: PromptXConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: PromptXConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: PromptXConfig): boolean => {
