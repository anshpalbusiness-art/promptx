...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.PROMPTX_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-test-"));
    process.env.PROMPTX_STATE_DIR = stateDir;
    process.env.PROMPTX_PROFILE = "isolated";

    try {
      const tool = createPromptXTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: promptx --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/promptx" } }\n}\n';
