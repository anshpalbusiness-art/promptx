...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "promptx",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/promptx.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("promptx");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "promptx");
