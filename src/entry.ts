...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.PROMPTX_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.PROMPTX_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.PROMPTX_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[promptx] ${parsed.error}`);
    process.exit(2);
  }
