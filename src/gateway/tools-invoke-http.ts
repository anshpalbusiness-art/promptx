...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-promptx-message-channel") ?? "");
  const accountId = getHeader(req, "x-promptx-account-id")?.trim() || undefined;
  ...
  const allTools = createPromptXTools({
