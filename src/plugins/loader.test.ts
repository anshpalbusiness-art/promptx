...
import { loadPromptXPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.PROMPTX_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `promptx-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "promptx.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.PROMPTX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadPromptXPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.PROMPTX_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadPromptXPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadPromptXPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
