...
Tlon is a decentralized messenger built on Urbit. PromptX connects to your Urbit ship and can
...
```bash
promptx plugins install @promptx/tlon
```

Local checkout (when running from a git repo):

```bash
promptx plugins install ./extensions/tlon
```
...
Use these with `promptx message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, PromptX replies in-thread.
