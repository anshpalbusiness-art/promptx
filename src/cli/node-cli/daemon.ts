...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("promptx node install"), formatCliCommand("promptx node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("promptx node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    PROMPTX_LOG_PREFIX: baseEnv.PROMPTX_LOG_PREFIX ?? "node",
