...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: PromptXOAuthStore.AnthropicOAuthStatus = PromptXOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: PromptXOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = PromptXOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = PromptXOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try PromptXOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. PromptX can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: PromptXOAuthStore.AnthropicOAuthStatus,
