...
struct ChatMessageBubble: View {
    let message: PromptXChatMessage
    let style: PromptXChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? PromptXChatTheme.userText : PromptXChatTheme.assistantText
    ...
    private var inlineAttachments: [PromptXChatMessageContent] {
    ...
    private var toolCalls: [PromptXChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [PromptXChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? PromptXChatTheme.userBubble
        }
        if self.style == .onboarding {
            return PromptXChatTheme.onboardingAssistantBubble
        }
        return PromptXChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return PromptXChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: PromptXChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? PromptXChatTheme.userText : PromptXChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: PromptXChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PromptXChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(PromptXChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [PromptXChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: PromptXChatTheme.assistantText)
            }
        }
    }
}
