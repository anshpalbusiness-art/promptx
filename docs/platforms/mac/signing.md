...
- sets a stable debug bundle identifier: `ai.promptx.mac.debug`
...
- inject build metadata into Info.plist: `PromptXBuildTimestamp` (UTC) and `PromptXGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `PromptXBuildTimestamp`: ISO8601 UTC at package time
- `PromptXGitCommit`: short git hash (or `unknown` if unavailable)
...
TCC permissions are tied to the bundle identifier _and_ code signature. Unsigned debug builds with changing UUIDs were causing macOS to forget grants after each rebuild. Signing the binaries (ad‑hoc by default) and keeping a fixed bundle id/path (`dist/PromptX.app`) preserves the grants between builds, matching the VibeTunnel approach.
