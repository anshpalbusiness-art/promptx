...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "promptx-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "promptx-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "promptx-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "promptx-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.promptx when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "promptx-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers PROMPTX_HOME over HOME", () => {
    vi.stubEnv("PROMPTX_HOME", "/srv/promptx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/promptx-home"));
...
describe("shortenHomePath", () => {
  it("uses $PROMPTX_HOME prefix when PROMPTX_HOME is set", () => {
    vi.stubEnv("PROMPTX_HOME", "/srv/promptx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/promptx-home")}/.promptx/promptx.json`)).toBe(
      "$PROMPTX_HOME/.promptx/promptx.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $PROMPTX_HOME replacement when PROMPTX_HOME is set", () => {
    vi.stubEnv("PROMPTX_HOME", "/srv/promptx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/promptx-home")}/.promptx/promptx.json`),
    ).toBe("config: $PROMPTX_HOME/.promptx/promptx.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/promptx")).toBe(path.resolve(os.homedir(), "promptx"));
  });
  ...
  it("prefers PROMPTX_HOME for tilde expansion", () => {
    vi.stubEnv("PROMPTX_HOME", "/srv/promptx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/promptx")).toBe(path.resolve("/srv/promptx-home", "promptx"));
