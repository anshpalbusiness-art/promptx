---
summary: "Use MiniMax M2.1 in PromptX"
read_when:
  - You want MiniMax models in PromptX
...
```bash
promptx plugins enable minimax-portal-auth  # skip if already loaded.
promptx gateway restart  # restart if gateway is already running
promptx onboard --auth-choice minimax-portal
```
...
Configure manually via `promptx.json`:
...
- Use `promptx models list` and `promptx models set minimax/MiniMax-M2.1` to switch.
...
- Running `promptx configure` and selecting **MiniMax M2.1**, or
