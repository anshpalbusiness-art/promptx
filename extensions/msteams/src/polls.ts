...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["promptxPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["promptx", "pollId"]) ??
    readNestedString(value, ["promptx", "poll", "id"]) ??
    readNestedString(value, ["data", "promptxPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "promptx", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          promptxPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "promptx poll vote",
          displayText: "Vote recorded",
          value: { promptxPollId: pollId, pollId },
        },
      },
    ],
  };
