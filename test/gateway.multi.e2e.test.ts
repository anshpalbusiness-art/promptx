...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `promptx-e2e-${name}-`));
  const configDir = path.join(homeDir, ".promptx");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          PROMPTX_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        PROMPTX_GATEWAY_TOKEN: inst.gatewayToken,
        PROMPTX_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          PROMPTX_GATEWAY_PORT: String(gwA.port),
          PROMPTX_GATEWAY_TOKEN: gwA.gatewayToken,
          PROMPTX_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          PROMPTX_GATEWAY_PORT: String(gwB.port),
          PROMPTX_GATEWAY_TOKEN: gwB.gatewayToken,
          PROMPTX_GATEWAY_PASSWORD: "",
        }),
