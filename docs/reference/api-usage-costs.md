...
PromptX features that can generate provider usage or paid API calls.
...
- `promptx status --usage` and `promptx channels list` show provider **usage windows**
...
PromptX can pick up credentials from:
...
- `promptx status --usage`
- `promptx models status --json`
...
`promptx models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
