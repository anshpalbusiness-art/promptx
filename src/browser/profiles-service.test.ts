...
vi.mock("./chrome.js", () => ({
  resolvePromptXUserDataDir: vi.fn(() => "/tmp/promptx-test/promptx/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolvePromptXUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "promptx-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolvePromptXUserDataDir).mockReturnValue(userDataDir);
