---
summary: "PromptX on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up PromptX on a Raspberry Pi
  - Running PromptX on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# PromptX on Raspberry Pi

## Goal

Run a persistent, always-on PromptX Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most PromptX features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled promptx

# Enable if not
sudo systemctl enable promptx

# Start on boot
sudo systemctl start promptx
```
...
```bash
# Check logs
journalctl -u promptx --no-pager -n 100

# Common fix: rebuild
cd ~/promptx  # if using hackable install
npm run build
sudo systemctl restart promptx
```
