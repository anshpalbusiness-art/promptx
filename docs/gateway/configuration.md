---
summary: "All configuration options for ~/.promptx/promptx.json with examples"
read_when:
...
PromptX reads an optional **JSON5** config from `~/.promptx/promptx.json` (comments + trailing commas allowed).

If the file is missing, PromptX uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.promptx/workspace`). You usually only need a config to:
...
PromptX only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `promptx doctor`, `promptx logs`, `promptx health`, `promptx status`, `promptx service`, `promptx help`).
- Run `promptx doctor` to see the exact issues.
- Run `promptx doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `promptx config set`. Keep a backup of `~/.promptx/promptx.json`.
...
```bash
promptx gateway call config.get --params '{}' # capture payload.hash
promptx gateway call config.apply --params '{
...
```bash
promptx gateway call config.get --params '{}' # capture payload.hash
promptx gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.promptx/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@promptx", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.promptx/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.promptx/workspace" }],
}
```
...
{ "$include": "/etc/promptx/base.json5" } // absolute
...
```json5
// ~/.promptx/clients/mueller/agents.json5
[
...
```json5
// ~/.promptx/clients/mueller/broadcast.json5
{
...
PromptX reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, PromptX runs your login shell and imports only the missing expected keys (never overrides).
...
- `PROMPTX_LOAD_SHELL_ENV=1`
- `PROMPTX_SHELL_ENV_TIMEOUT_MS=15000`
...
PromptX stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.promptx/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.promptx/credentials/oauth.json` (or `$PROMPTX_STATE_DIR/credentials/oauth.json`)
...
- `~/.promptx/agent/*` (migrated by `promptx doctor` into `~/.promptx/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `PROMPTX_OAUTH_DIR`
- Agent dir (default agent root override): `PROMPTX_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, PromptX imports `oauth.json` entries into `auth-profiles.json`.
...
If set, PromptX derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/promptx/promptx-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/promptx/promptx.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.promptx/credentials/whatsapp/biz
          // authDir: "~/.promptx/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `promptx doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@promptx", "promptx"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@promptx"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.promptx/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.promptx/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.promptx/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.promptx/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.promptx/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.promptx/workspace-home" },
      { id: "work", workspace: "~/.promptx/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `promptx.json`).
...
PromptX starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["promptx-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-promptx",
...
PromptX starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `promptx plugins install @promptx/mattermost` (or `./extensions/mattermost` from a git checkout).
...
PromptX starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
PromptX spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, PromptX
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/promptx" } },
}
```
...
When a file exceeds this limit, PromptX logs a warning and injects a truncated
...
message envelopes). If unset, PromptX uses the host timezone at runtime.
...
`[promptx]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[promptx]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[promptx]"`, PromptX will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, PromptX generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.promptx/settings/tts.json",
      elevenlabs: {
...
PromptX also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, PromptX currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `promptx configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:promptx`: all built-in PromptX tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.promptx/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.promptx/sandboxes",
        docker: {
          ...
          apparmorProfile: "promptx-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "promptx-sandbox-browser:bookworm-slim",
          containerPrefix: "promptx-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
PromptX uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.promptx/agents/<agentId>/agent/models.json` or by defining the same schema inside your
PromptX config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, PromptX writes/merges a `models.json` into
`~/.promptx/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. PromptX uses
...
- Shortcut: `promptx onboard --auth-choice opencode-zen`.
...
Shortcut: `promptx onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `promptx onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `promptx onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `promptx onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `promptx onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `promptx onboard --auth-choice minimax-api`.
...
- Override the agent config root with `PROMPTX_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.promptx/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.promptx/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.promptx/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, PromptX stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.promptx/skills` (workspace skills
...
from `~/.promptx/extensions`, `<workspace>/.promptx/extensions`, plus any
...
### `browser` (promptx-managed browser)

PromptX can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for promptx and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (PromptX.app menubar, or `promptx gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      promptx: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/promptx" }
...
- Examples: `"/ui"`, `"/promptx"`, `"/apps/promptx"`.
...
- When a connection comes from one of these IPs, PromptX uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `promptx gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `PROMPTX_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `PROMPTX_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. PromptX
...
- PromptX.app watches `~/.promptx/promptx.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.promptx/promptx.json` (or `PROMPTX_CONFIG_PATH`) and applies changes automatically.
...
- `~/.promptx/promptx.json` (or `PROMPTX_CONFIG_PATH`)
...
- `PROMPTX_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `promptx --dev …` → uses `~/.promptx-dev` + shifts ports from base `19001`
- `promptx --profile <name> …` → uses `~/.promptx-<name>` (port via config/env/flags)
...
```bash
PROMPTX_CONFIG_PATH=~/.promptx/a.json \
PROMPTX_STATE_DIR=~/.promptx-a \
promptx gateway --port 19001
```
...
Gmail helper config (used by `promptx webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "promptx@gmail.com",
...
- Set `PROMPTX_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, PromptX defaults `serve.path` to `/` so
...
Default root: `~/.promptx/workspace/canvas`  
Default port: `18793` (chosen to avoid the promptx browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__promptx__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__promptx__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `PROMPTX_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.promptx/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.promptx/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.promptx/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_promptx-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `promptx` (advertises `promptx.local`). Override with `PROMPTX_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_promptx-gw._tcp` under `~/.promptx/dns/` using the configured discovery domain (example: `promptx.internal.`).
