...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.promptx.gateway"
    private static let nodeService = "ai.promptx.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.promptx.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.promptx.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("promptx-gateway.log")
    }
