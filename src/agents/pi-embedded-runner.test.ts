...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-embedded-agent-"));
...
const ensureModels = (cfg: PromptXConfig) => ensurePromptXModelsJson(cfg, agentDir) as unknown;
