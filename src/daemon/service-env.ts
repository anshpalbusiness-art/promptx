...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.PROMPTX_STATE_DIR;
  const configPath = env.PROMPTX_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    PROMPTX_PROFILE: profile,
    PROMPTX_STATE_DIR: stateDir,
    PROMPTX_CONFIG_PATH: configPath,
    PROMPTX_GATEWAY_PORT: String(port),
    PROMPTX_GATEWAY_TOKEN: token,
    PROMPTX_LAUNCHD_LABEL: resolvedLaunchdLabel,
    PROMPTX_SYSTEMD_UNIT: systemdUnit,
    PROMPTX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    PROMPTX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    PROMPTX_STATE_DIR: stateDir,
    PROMPTX_CONFIG_PATH: configPath,
    PROMPTX_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    PROMPTX_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    PROMPTX_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    PROMPTX_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    PROMPTX_LOG_PREFIX: "node",
    PROMPTX_SERVICE_MARKER: NODE_SERVICE_MARKER,
    PROMPTX_SERVICE_KIND: NODE_SERVICE_KIND,
    PROMPTX_SERVICE_VERSION: VERSION,
  };
}
