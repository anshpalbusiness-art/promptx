...
Deploy PromptX on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/promptx` — Control UI
...
- `PROMPTX_STATE_DIR=/data/.promptx` (recommended)
- `PROMPTX_WORKSPACE_DIR=/data/workspace` (recommended)
- `PROMPTX_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your PromptX state + workspace so you can migrate to another host without losing config or memory.
