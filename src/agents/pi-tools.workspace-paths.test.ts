...
import { createPromptXCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("promptx-ws-", async (workspaceDir) => {
      await withTempDir("promptx-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createPromptXCodingTools({ workspaceDir });
    ...
    await withTempDir("promptx-ws-", async (workspaceDir) => {
      await withTempDir("promptx-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello promptx");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("promptx-ws-", async (workspaceDir) => {
      const tools = createPromptXCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("promptx-ws-", async (workspaceDir) => {
      await withTempDir("promptx-override-", async (overrideDir) => {
        const tools = createPromptXCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("promptx-sandbox-", async (sandboxDir) => {
      await withTempDir("promptx-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "promptx-sbx-test",
        ...
        const tools = createPromptXCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
