...
import { scheduleGatewaySigusr1Restart, triggerPromptXRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting PromptX in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPromptXRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting PromptX via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
