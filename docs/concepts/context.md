...
read_when:
  - You want to understand what “context” means in PromptX
...
“Context” is **everything PromptX sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (PromptX-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How PromptX builds the system prompt

The system prompt is **PromptX-owned** and rebuilt each run. It includes:
...
By default, PromptX injects a fixed set of workspace files (if present):
