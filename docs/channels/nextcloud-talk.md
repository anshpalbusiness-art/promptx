...
```bash
promptx plugins install @promptx/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
promptx plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "PromptX" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `promptx pairing list nextcloud-talk`
  - `promptx pairing approve nextcloud-talk <CODE>`
