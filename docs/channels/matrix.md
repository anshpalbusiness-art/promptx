...
Matrix is an open, decentralized messaging protocol. PromptX connects as a Matrix **user**
...
```bash
promptx plugins install @promptx/matrix
```

Local checkout (when running from a git repo):

```bash
promptx plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `promptx plugins install @promptx/matrix`
   - From a local checkout: `promptx plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: PromptX calls the same
     login endpoint, stores the access token in `~/.promptx/credentials/matrix/credentials.json`,
...
- On first connection, PromptX requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  PromptX logs a warning.
...
`~/.promptx/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `promptx pairing list matrix`
  - `promptx pairing approve matrix <CODE>`
...
- On startup, PromptX resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
