...
你可以在 `~/.promptx/promptx.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/promptx.log"
  }
}
```
...
```bash
promptx channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.promptx/promptx.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/promptx/promptx-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- PromptX 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
PROMPTX_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `promptx plugins enable diagnostics-otel` 启用插件。
...
- `promptx.tokens`（计数器，属性：`promptx.token`、`promptx.channel`、`promptx.provider`、`promptx.model`）
- `promptx.cost.usd`（计数器，属性：`promptx.channel`、`promptx.provider`、`promptx.model`）
- `promptx.run.duration_ms`（直方图，属性：`promptx.channel`、`promptx.provider`、`promptx.model`）
- `promptx.context.tokens`（直方图，属性：`promptx.context`、`promptx.channel`、`promptx.provider`、`promptx.model`）

消息流：

- `promptx.webhook.received`（计数器，属性：`promptx.channel`、`promptx.webhook`）
- `promptx.webhook.error`（计数器，属性：`promptx.channel`、`promptx.webhook`）
- `promptx.webhook.duration_ms`（直方图，属性：`promptx.channel`、`promptx.webhook`）
- `promptx.message.queued`（计数器，属性：`promptx.channel`、`promptx.source`）
- `promptx.message.processed`（计数器，属性：`promptx.channel`、`promptx.outcome`）
- `promptx.message.duration_ms`（直方图，属性：`promptx.channel`、`promptx.outcome`）

队列 + 会话：

- `promptx.queue.lane.enqueue`（计数器，属性：`promptx.lane`）
- `promptx.queue.lane.dequeue`（计数器，属性：`promptx.lane`）
- `promptx.queue.depth`（直方图，属性：`promptx.lane` 或 `promptx.channel=heartbeat`）
- `promptx.queue.wait_ms`（直方图，属性：`promptx.lane`）
- `promptx.session.state`（计数器，属性：`promptx.state`、`promptx.reason`）
- `promptx.session.stuck`（计数器，属性：`promptx.state`）
- `promptx.session.stuck_age_ms`（直方图，属性：`promptx.state`）
- `promptx.run.attempt`（计数器，属性：`promptx.attempt`）

### 导出的 span（名称 + 关键属性）

- `promptx.model.usage`
  - `promptx.channel`、`promptx.provider`、`promptx.model`
  - `promptx.sessionKey`、`promptx.sessionId`
  - `promptx.tokens.*`（input/output/cache_read/cache_write/total）
- `promptx.webhook.processed`
  - `promptx.channel`、`promptx.webhook`、`promptx.chatId`
- `promptx.webhook.error`
  - `promptx.channel`、`promptx.webhook`、`promptx.chatId`、`promptx.error`
- `promptx.message.processed`
  - `promptx.channel`、`promptx.outcome`、`promptx.chatId`、`promptx.messageId`、`promptx.sessionKey`、`promptx.sessionId`、`promptx.reason`
- `promptx.session.stuck`
  - `promptx.state`、`promptx.ageMs`、`promptx.queueDepth`、`promptx.sessionKey`、`promptx.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `promptx doctor`。
