...
export function triggerPromptXRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.PROMPTX_SYSTEMD_UNIT,
        process.env.PROMPTX_PROFILE,
      );
  ...
  const label =
    process.env.PROMPTX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.PROMPTX_PROFILE);
