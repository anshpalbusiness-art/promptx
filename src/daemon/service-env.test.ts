...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.PROMPTX_GATEWAY_PORT).toBe("18789");
    expect(env.PROMPTX_GATEWAY_TOKEN).toBe("secret");
    expect(env.PROMPTX_SERVICE_MARKER).toBe("promptx");
    expect(env.PROMPTX_SERVICE_KIND).toBe("gateway");
    expect(typeof env.PROMPTX_SERVICE_VERSION).toBe("string");
    expect(env.PROMPTX_SYSTEMD_UNIT).toBe("promptx-gateway.service");
    if (process.platform === "darwin") {
      expect(env.PROMPTX_LAUNCHD_LABEL).toBe("ai.promptx.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", PROMPTX_PROFILE: "work" },
      port: 18789,
    });
    expect(env.PROMPTX_SYSTEMD_UNIT).toBe("promptx-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.PROMPTX_LAUNCHD_LABEL).toBe("ai.promptx.work");
    }
  });
});
