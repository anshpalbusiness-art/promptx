...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "PROMPTX_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("PROMPTX_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv PROMPTX_GATEWAY_TOKEN");
    expect(message).not.toContain("PROMPTX_GATEWAY_PASSWORD");
  });
