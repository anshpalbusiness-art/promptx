...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "promptx.mjs");
    ...
    const doctorStep = await runStep(
      step("promptx doctor", doctorArgv, gitRoot, { PROMPTX_UPDATE_IN_PROGRESS: "1" }),
    );
