...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/promptx/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "PromptX Re-Auth" -c "After getting key, run: ssh $SERVER '~/promptx/scripts/mobile-reauth.sh'" --id promptx-auth
        ;;
    *)
