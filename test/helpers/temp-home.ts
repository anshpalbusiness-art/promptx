...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("PROMPTX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.PROMPTX_STATE_DIR = path.join(base, ".promptx");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "promptx-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".promptx", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
