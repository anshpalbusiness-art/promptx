...
const requireAgentDir = () => {
  const agentDir = process.env.PROMPTX_AGENT_DIR;
  if (!agentDir) {
    throw new Error("PROMPTX_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.PROMPTX_OAUTH_DIR;
  });

  it("writes auth-profiles.json under PROMPTX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-oauth-"));
    process.env.PROMPTX_STATE_DIR = tempStateDir;
    process.env.PROMPTX_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.PROMPTX_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to PROMPTX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-minimax-"));
    process.env.PROMPTX_STATE_DIR = tempStateDir;
    process.env.PROMPTX_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.PROMPTX_AGENT_DIR;
