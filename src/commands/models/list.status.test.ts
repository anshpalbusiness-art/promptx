...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePromptXAgentDir: vi.fn().mockReturnValue("/tmp/promptx-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/promptx-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/promptx-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePromptXAgentDir: mocks.resolvePromptXAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolvePromptXAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/promptx-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/promptx-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/promptx-agent-custom");
