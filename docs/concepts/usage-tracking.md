...
- `/usage cost` in chats: local cost summary aggregated from PromptX session logs.
- CLI: `promptx status --usage` prints a full per-provider breakdown.
- CLI: `promptx channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
