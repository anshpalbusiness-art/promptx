...
function resolveExecConfig(cfg: PromptXConfig | undefined) {
...
export function createPromptXCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createPromptXReadTool(freshReadTool)];
    }
