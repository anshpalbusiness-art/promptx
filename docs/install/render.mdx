...
Deploy PromptX on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
repository configures everything needed to run PromptX:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: PROMPTX_STATE_DIR
        value: /data/.promptx
      - key: PROMPTX_WORKSPACE_DIR
        value: /data/workspace
      - key: PROMPTX_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/promptx`.
...
If you use the original PromptX repository, Render will not auto-deploy your PromptX. To update it, run a manual Blueprint sync from the dashboard.
...
For PromptX, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any PromptX host.
