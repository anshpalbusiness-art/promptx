...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "promptx-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      PROMPTX_NO_RESPAWN: "1",
      PROMPTX_STATE_DIR: stateDir,
      PROMPTX_SKIP_CHANNELS: "1",
      PROMPTX_SKIP_GMAIL_WATCHER: "1",
      PROMPTX_SKIP_CRON: "1",
      PROMPTX_SKIP_BROWSER_CONTROL_SERVER: "1",
      PROMPTX_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "promptx-entry-bootstrap.mjs");
