...
beforeAll(() => {
  previousProfile = process.env.PROMPTX_PROFILE;
  process.env.PROMPTX_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.PROMPTX_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/promptx-root.js", () => ({
  resolvePromptXPackageRoot: vi.fn().mockResolvedValue("/tmp/promptx"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/promptx/pnpm-lock.yaml",
      markerPath: "/tmp/promptx/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("PromptX status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("promptx status --all") ||
          l.includes("promptx --profile isolated status --all") ||
          l.includes("promptx status --all") ||
          l.includes("promptx --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.PROMPTX_GATEWAY_TOKEN = "abcd1234";
    try {
