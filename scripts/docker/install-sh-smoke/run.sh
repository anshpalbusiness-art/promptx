...
INSTALL_URL="${PROMPTX_INSTALL_URL:-https://openclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${PROMPTX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${PROMPTX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="promptx"
PACKAGE_NAME="${PROMPTX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (PROMPTX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${PROMPTX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${PROMPTX_INSTALL_LATEST_OUT:-}"
fi
