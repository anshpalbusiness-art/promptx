...
IMAGE_NAME="promptx-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    PROMPTX_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    PROMPTX_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export PROMPTX_ENTRY

	  home_dir=$(mktemp -d "/tmp/promptx-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.promptx/extensions/demo-plugin"

  cat > "$HOME/.promptx/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.promptx/extensions/demo-plugin/promptx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$PROMPTX_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/promptx-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@promptx/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/promptx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$PROMPTX_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$PROMPTX_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/promptx-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@promptx/demo-plugin-dir",
  "version": "0.0.1",
  "promptx": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/promptx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$PROMPTX_ENTRY" plugins install "$dir_plugin"
	  node "$PROMPTX_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/promptx-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@promptx/demo-plugin-file",
  "version": "0.0.1",
  "promptx": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/promptx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$PROMPTX_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$PROMPTX_ENTRY" plugins list --json > /tmp/plugins4.json
