...
import type { PromptXPluginApi, PromptXPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "promptx-lobster-plugin-") {
...
function fakeApi(overrides: Partial<PromptXPluginApi> = {}): PromptXPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<PromptXPluginToolContext> = {}): PromptXPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: PromptXPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
