...
- During heartbeat runs, PromptX treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `promptx sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, PromptX skips the heartbeat run entirely (no model call).
...
```bash
promptx system event --text "Check for urgent follow-ups" --mode now
```
