...
# Build and bundle PromptX into a minimal .app we can open.
# Outputs to dist/PromptX.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/PromptX.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="PromptX"
BUNDLE_ID="${BUNDLE_ID:-ai.promptx.mac.debug}"
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/PromptX/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :PromptXBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :PromptXGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/PromptX"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/PromptX"
fi
chmod +x "$APP_ROOT/Contents/MacOS/PromptX"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/PromptX" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/PromptX/Resources/PromptX.icns" "$APP_ROOT/Contents/Resources/PromptX.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/PromptX/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying PromptXKit resources"
PROMPTXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/PromptXKit_PromptXKit.bundle"
if [ -d "$PROMPTXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/PromptXKit_PromptXKit.bundle"
  cp -R "$PROMPTXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/PromptXKit_PromptXKit.bundle"
else
  echo "WARN: PromptXKit resource bundle not found at $PROMPTXKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running PromptX"
killall -q PromptX 2>/dev/null || true
