import type { BrowserConfig, BrowserProfileConfig, PromptXConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_PROMPTX_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_PROMPTX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "promptx" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_PROMPTX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_PROMPTX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an PromptX browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for promptx/promptx CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: PromptXConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_PROMPTX_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "promptx";
