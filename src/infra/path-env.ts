...
function candidateBinDirs(opts: EnsurePromptXPathOpts): string[] {
  ...
  // Bundled macOS app: `promptx` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "promptx");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/promptx` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "promptx"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `promptx` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensurePromptXCliOnPath(opts: EnsurePromptXPathOpts = {}) {
  if (isTruthyEnvValue(process.env.PROMPTX_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.PROMPTX_PATH_BOOTSTRAPPED = "1";
