...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/promptx`)
...
If the page fails to load, start the Gateway first: `promptx gateway`.
...
you revoke it with `promptx devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.promptx/promptx.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. PromptX
...
PromptX **blocks** Control UI connections without device identity.
