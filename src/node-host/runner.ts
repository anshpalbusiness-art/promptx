...
const execHostEnforced = process.env.PROMPTX_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.PROMPTX_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensurePromptXCliOnPath({ pathEnv: process.env.PATH ?? "" });
