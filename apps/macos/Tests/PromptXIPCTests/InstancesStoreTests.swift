...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: PromptXProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: PromptXProtocol.AnyCodable] = [
            "presence": .init([PromptXProtocol.AnyCodable(entry)]),
        ]
        let payload = PromptXProtocol.AnyCodable(payloadMap)
