...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("promptxCanvasA2UIAction");
    expect(out).toContain("promptxSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("promptxSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.promptxA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__promptx__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("promptx-a2ui-host");
      expect(html).toContain("promptxCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__promptx__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("promptxA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
