...
function resolvePromptXRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.PROMPTX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set PROMPTX_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolvePromptXRoot(), "dist", "extensionAPI.js");
