...
show_usage() {
    cat << EOF
clawlog - PromptX Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View PromptX logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    PromptX logs flow through the macOS unified log (subsystem: ai.promptx).
