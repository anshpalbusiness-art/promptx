...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.promptx/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.promptx/node.json` (node id + pairing token).
...
Path: `~/.promptx/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.promptx/exec-approvals.sock",
...
- Unix socket at `~/.promptx/exec-approvals.sock` (0600).
