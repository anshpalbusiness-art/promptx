...
- Replaces the legacy `gateway` command. CLI entry point: `promptx gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
promptx gateway --port 18789
# for full debug/trace logs in stdio:
promptx gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
promptx gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.promptx/promptx.json` (or `PROMPTX_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__promptx__/canvas/` from `~/.promptx/workspace/canvas`. Disable with `canvasHost.enabled=false` or `PROMPTX_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `PROMPTX_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `PROMPTX_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.promptx.*` may still exist)
- Linux: `promptx-gateway-<profile>.service`
- Windows: `PromptX Gateway (<profile>)`

Install metadata is embedded in the service config:

- `PROMPTX_SERVICE_MARKER=promptx`
- `PROMPTX_SERVICE_KIND=gateway`
- `PROMPTX_SERVICE_VERSION=<version>`
...
```bash
promptx --dev setup
promptx --dev gateway --allow-unconfigured
...
- `PROMPTX_STATE_DIR=~/.promptx-dev`
- `PROMPTX_CONFIG_PATH=~/.promptx-dev/promptx.json`
- `PROMPTX_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.promptx/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `PROMPTX_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `PROMPTX_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `PROMPTX_CONFIG_PATH`
- unique `PROMPTX_STATE_DIR`
...
```bash
promptx --profile main gateway install
promptx --profile rescue gateway install
```

Example:

```bash
PROMPTX_CONFIG_PATH=~/.promptx/a.json PROMPTX_STATE_DIR=~/.promptx-a promptx gateway --port 19001
PROMPTX_CONFIG_PATH=~/.promptx/b.json PROMPTX_STATE_DIR=~/.promptx-b promptx gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `promptx health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `promptx`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `promptx gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.promptx.*` is cleaned up).
  - `promptx doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are PromptX profile services.
  ...
  - Cleanup: `promptx gateway uninstall` (current service) and `promptx doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `promptx gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- PromptX.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.promptx.*` labels still unload cleanly).
- To stop it cleanly, use `promptx gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `promptx gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `promptx gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

PromptX installs a **systemd user service** by default on Linux/WSL2. We
...
`promptx gateway install` writes the user unit. `promptx doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/promptx-gateway[-<profile>].service`:

```
[Unit]
Description=PromptX Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/promptx gateway --port 18789
Restart=always
RestartSec=5
Environment=PROMPTX_GATEWAY_TOKEN=
...
```
systemctl --user enable --now promptx-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/promptx-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now promptx-gateway[-<profile>].service
```
...
- `promptx gateway health|status` — request health/status over the Gateway WS.
- `promptx message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `promptx agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `promptx gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `promptx gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `promptx gateway` and the legacy TCP control port.
