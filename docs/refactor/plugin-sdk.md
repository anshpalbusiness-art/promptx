...
- Publish as `promptx/plugin-sdk` (or export from core under `promptx/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `PromptXPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: PromptXConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: PromptXConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: PromptXConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: PromptXConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: PromptXConfig): string;
  };
};
```
...
- Introduce `promptx/plugin-sdk`.
- Add `api.runtime` to `PromptXPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `promptxRuntime: ">=2026.2.0"`).
