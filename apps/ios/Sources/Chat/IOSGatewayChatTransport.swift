...
struct IOSGatewayChatTransport: PromptXChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PromptXChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(PromptXChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> PromptXChatHistoryPayload {
        ...
        return try JSONDecoder().decode(PromptXChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [PromptXChatAttachmentPayload]) async throws -> PromptXChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [PromptXChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(PromptXChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(PromptXGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PromptXGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PromptXChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PromptXAgentEventPayload.self)
                        {
