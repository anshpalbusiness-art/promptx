...
describe("resolveEffectiveHomeDir", () => {
  it("prefers PROMPTX_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/promptx-home"));
  });
  ...
  it("expands PROMPTX_HOME when set to ~", () => {
    const env = {
      PROMPTX_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for PROMPTX_HOME", () => {
    const result = resolveRequiredHomeDir(
      { PROMPTX_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when PROMPTX_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ PROMPTX_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { PROMPTX_HOME: "/srv/promptx-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/promptx-home")}/x`);
  });
