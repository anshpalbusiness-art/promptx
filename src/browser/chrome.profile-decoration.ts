...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".promptx-profile-decorated");
}
 ...
 */
export function decoratePromptXProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_PROMPTX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_PROMPTX_BROWSER_COLOR).toUpperCase();
