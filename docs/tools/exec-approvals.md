...
- **gateway host** → `promptx` process on the gateway machine
...
`~/.promptx/exec-approvals.json` directly.

CLI: `promptx approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
