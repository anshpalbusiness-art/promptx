...
${formatHelpExamples([
  ['promptx agent --to +15555550123 --message "status update"', "Start a new session."],
  ['promptx agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'promptx agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'promptx agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['promptx agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'promptx agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.promptx.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.promptx.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "promptx agents set-identity --identity-file ~/.promptx/workspace/IDENTITY.md --agent main",
