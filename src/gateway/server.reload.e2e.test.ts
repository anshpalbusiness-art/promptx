...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.PROMPTX_SKIP_CHANNELS;
    prevSkipGmail = process.env.PROMPTX_SKIP_GMAIL_WATCHER;
    process.env.PROMPTX_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.PROMPTX_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.PROMPTX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.PROMPTX_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
