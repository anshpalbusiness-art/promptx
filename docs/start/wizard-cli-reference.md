...
read_when:
  - You need detailed behavior for promptx onboard
...
This page is the full reference for `promptx onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.promptx/promptx.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `promptx doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `promptx pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `promptx health`.
    - `promptx status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.promptx/.env` so launchd can read it.
...
- OAuth credentials: `~/.promptx/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.promptx/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`promptx onboard`](/cli/onboard)
