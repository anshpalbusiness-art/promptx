...
beforeEach(() => {
  ...
  resolvePromptXPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.PROMPTX_STATE_DIR;
  originalUpdateInProgress = process.env.PROMPTX_UPDATE_IN_PROGRESS;
  process.env.PROMPTX_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "promptx-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.PROMPTX_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.PROMPTX_UPDATE_IN_PROGRESS;
  } else {
    process.env.PROMPTX_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolvePromptXPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadPromptXPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/promptx-root.js", () => ({
  resolvePromptXPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "promptx-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.PROMPTX_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when PROMPTX_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/promptx.json",
      exists: true,
      raw: "{}",
    ...
    process.env.PROMPTX_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
