...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PromptXProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.promptx", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PromptXKit.AnyCodable]()) {
                $0[$1.key] = PromptXKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: PromptXProtocol.AnyCodable?) -> [String: PromptXProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PromptXProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PromptXKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PromptXProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("promptx.control.heartbeat")
    static let controlAgentEvent = Notification.Name("promptx.control.agent")
}
