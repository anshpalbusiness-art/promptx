...
`promptx.invoke --each`) without adding new PromptX code per workflow.
...
This extension depends on PromptX internal modules (the embedded agent runner).
It is intended to ship as a **bundled** PromptX extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.promptx/extensions` as a standalone plugin directory.
