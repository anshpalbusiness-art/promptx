...
final class RemindersService: RemindersServicing {
    func list(params: PromptXRemindersListParams) async throws -> PromptXRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[PromptXReminderPayload], Error>) in
        ...
        return PromptXRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: PromptXRemindersAddParams) async throws -> PromptXRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = PromptXReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return PromptXRemindersAddPayload(reminder: payload)
    }
