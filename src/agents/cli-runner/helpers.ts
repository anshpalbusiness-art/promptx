...
/**
 * Cleanup suspended PromptX CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: PromptXConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-cli-images-"));
  const paths: string[] = [];
