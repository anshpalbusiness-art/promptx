...
PromptX integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- PromptX owns lifecycle when `channels.signal.autoStart=true`.
...
- PromptX spawns `imsg rpc` as a child process (legacy iMessage integration).
