...
read_when:
  - You want PromptX to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables PromptX to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`promptx onboard`) and `promptx channels add` list optional channel plugins.
...
```bash
promptx plugins install @promptx/nostr
```

Use a local checkout (dev workflows):

```bash
promptx plugins install --link <path-to-promptx>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "promptx@example.com",
        "lud16": "promptx@example.com"
      }
    }
  }
}
