...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.promptx", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PromptXSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PromptXSessionPreviewEntry,
    ...
    private static func cache(payload: PromptXSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PromptXChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PromptXKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PromptXKit.AnyCodable]) -> [PromptXChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PromptXChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PromptXChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PromptXChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PromptXChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PromptXChatMessage) -> String? {
