---
summary: "Chrome extension: let PromptX drive your existing Chrome tab"
read_when:
...
The PromptX Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate promptx-managed Chrome profile.
...
PromptX then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the PromptX release (npm package) as static files. There is no separate “build” step.

After upgrading PromptX:

- Re-run `promptx browser extension install` to refresh the installed files under your PromptX state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

PromptX ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want PromptX to control.
...
- `ON`: attached; PromptX can drive that tab.
...
Debugging: `promptx sandbox explain`
...
`promptx browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `promptx browser extension install` first to copy the extension to a stable location under your PromptX state directory.
...
- **This is not isolated** like the dedicated promptx-managed profile.
