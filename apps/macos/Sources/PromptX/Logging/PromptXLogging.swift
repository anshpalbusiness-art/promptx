...
enum PromptXLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = PromptXOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PromptXFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PromptXLogging.bootstrapIfNeeded()
        let label = PromptXLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PromptXOSLogHandler: LogHandler {
...
struct PromptXFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PromptXLogging.parseLabel(self.label)
