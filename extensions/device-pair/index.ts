...
import { approveDevicePairing, listDevicePairing } from "promptx/plugin-sdk";
...
function resolveGatewayPort(cfg: PromptXPluginApi["config"]): number {
  const envRaw =
    process.env.PROMPTX_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: PromptXPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: PromptXPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: PromptXPluginApi): Promise<ResolveUrlResult> {
