...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    PROMPTX_STATE_DIR: process.env.PROMPTX_STATE_DIR,
    PROMPTX_CONFIG_PATH: process.env.PROMPTX_CONFIG_PATH,
    PROMPTX_GATEWAY_PORT: process.env.PROMPTX_GATEWAY_PORT,
    PROMPTX_PROFILE: process.env.PROMPTX_PROFILE,
  };

  beforeEach(() => {
    process.env.PROMPTX_STATE_DIR = "/tmp/promptx-cli-state";
    process.env.PROMPTX_CONFIG_PATH = "/tmp/promptx-cli-state/promptx.json";
    delete process.env.PROMPTX_GATEWAY_PORT;
    delete process.env.PROMPTX_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.PROMPTX_STATE_DIR !== undefined) {
      process.env.PROMPTX_STATE_DIR = originalEnv.PROMPTX_STATE_DIR;
    } else {
      delete process.env.PROMPTX_STATE_DIR;
    }

    if (originalEnv.PROMPTX_CONFIG_PATH !== undefined) {
      process.env.PROMPTX_CONFIG_PATH = originalEnv.PROMPTX_CONFIG_PATH;
    } else {
      delete process.env.PROMPTX_CONFIG_PATH;
    }

    if (originalEnv.PROMPTX_GATEWAY_PORT !== undefined) {
      process.env.PROMPTX_GATEWAY_PORT = originalEnv.PROMPTX_GATEWAY_PORT;
    } else {
      delete process.env.PROMPTX_GATEWAY_PORT;
    }

    if (originalEnv.PROMPTX_PROFILE !== undefined) {
      process.env.PROMPTX_PROFILE = originalEnv.PROMPTX_PROFILE;
    } else {
      delete process.env.PROMPTX_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        PROMPTX_PROFILE: "dev",
        PROMPTX_STATE_DIR: "/tmp/promptx-daemon-state",
        PROMPTX_CONFIG_PATH: "/tmp/promptx-daemon-state/promptx.json",
        PROMPTX_GATEWAY_PORT: "19001",
      },
