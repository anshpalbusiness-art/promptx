...
metadata: { "promptx": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == PromptX `sessions_spawn`
- **File I/O** == PromptX `read`/`write`
- **Remote fetch** == PromptX `web_fetch` (or `exec` with curl when POST is required)
