...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.PROMPTX_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "promptx");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.promptx-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".promptx");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "promptx.json");
      ...
      try {
        ...
        process.env.PROMPTX_CONFIG_PATH = prev.configPath;
        process.env.PROMPTX_GATEWAY_TOKEN = prev.token;
        process.env.PROMPTX_SKIP_CHANNELS = prev.skipChannels;
        process.env.PROMPTX_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.PROMPTX_SKIP_CRON = prev.skipCron;
        process.env.PROMPTX_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.PROMPTX_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.PROMPTX_STATE_DIR = prev.stateDir;
      process.env.PROMPTX_CONFIG_PATH = prev.configPath;
      process.env.PROMPTX_GATEWAY_TOKEN = prev.token;
      process.env.PROMPTX_SKIP_CHANNELS = prev.skipChannels;
      process.env.PROMPTX_SKIP_GMAIL_WATCHER = prev.skipGmail;
