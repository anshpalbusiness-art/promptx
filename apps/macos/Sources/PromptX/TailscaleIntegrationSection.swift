...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.promptx/promptx.json. Prefer PROMPTX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.promptx/promptx.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.promptx/promptx.json. Restart the gateway to apply."
        }
