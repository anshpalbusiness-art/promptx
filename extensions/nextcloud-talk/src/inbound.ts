...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as PromptXConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as PromptXConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as PromptXConfig);
