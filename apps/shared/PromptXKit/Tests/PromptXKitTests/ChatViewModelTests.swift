...
private final class TestChatTransport: @unchecked Sendable, PromptXChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [PromptXChatHistoryPayload]
    private let sessionsResponses: [PromptXChatSessionsListResponse]

    private let stream: AsyncStream<PromptXChatTransportEvent>
    private let continuation: AsyncStream<PromptXChatTransportEvent>.Continuation

    init(
        historyResponses: [PromptXChatHistoryPayload],
        sessionsResponses: [PromptXChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<PromptXChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> PromptXChatHistoryPayload {
        ...
        return self.historyResponses.last ?? PromptXChatHistoryPayload(
        ...
        attachments _: [PromptXChatAttachmentPayload]) async throws -> PromptXChatSendResponse
    {
        ...
        return PromptXChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> PromptXChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? PromptXChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: PromptXChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = PromptXChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = PromptXChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { PromptXChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = PromptXChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = PromptXChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = PromptXChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { PromptXChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
