...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [PromptXCapability.canvas.rawValue, PromptXCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(PromptXCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(PromptXCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = PromptXLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(PromptXCapability.location.rawValue) }

        caps.append(PromptXCapability.device.rawValue)
        caps.append(PromptXCapability.photos.rawValue)
        caps.append(PromptXCapability.contacts.rawValue)
        caps.append(PromptXCapability.calendar.rawValue)
        caps.append(PromptXCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(PromptXCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            PromptXSystemCommand.notify.rawValue,
            PromptXChatCommand.push.rawValue,
        ...
        if caps.contains(PromptXCapability.camera.rawValue) {
            commands.append(PromptXCameraCommand.list.rawValue)
            commands.append(PromptXCameraCommand.snap.rawValue)
            commands.append(PromptXCameraCommand.clip.rawValue)
        }
        if caps.contains(PromptXCapability.location.rawValue) {
            commands.append(PromptXLocationCommand.get.rawValue)
        }
        if caps.contains(PromptXCapability.device.rawValue) {
            commands.append(PromptXDeviceCommand.status.rawValue)
            commands.append(PromptXDeviceCommand.info.rawValue)
        }
        if caps.contains(PromptXCapability.photos.rawValue) {
            commands.append(PromptXPhotosCommand.latest.rawValue)
        }
        if caps.contains(PromptXCapability.contacts.rawValue) {
            commands.append(PromptXContactsCommand.search.rawValue)
            commands.append(PromptXContactsCommand.add.rawValue)
        }
        if caps.contains(PromptXCapability.calendar.rawValue) {
            commands.append(PromptXCalendarCommand.events.rawValue)
            commands.append(PromptXCalendarCommand.add.rawValue)
        }
        if caps.contains(PromptXCapability.reminders.rawValue) {
            commands.append(PromptXRemindersCommand.list.rawValue)
            commands.append(PromptXRemindersCommand.add.rawValue)
        }
        if caps.contains(PromptXCapability.motion.rawValue) {
            commands.append(PromptXMotionCommand.activity.rawValue)
            commands.append(PromptXMotionCommand.pedometer.rawValue)
        }
