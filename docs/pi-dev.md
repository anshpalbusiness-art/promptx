...
This guide summarizes a sane workflow for working on the pi integration in PromptX.
...
- Trigger the agent directly:
  - `pnpm promptx agent --message "Hello" --thinking low`
...
State lives under the PromptX state directory. Default is `~/.promptx`. If `PROMPTX_STATE_DIR` is set, use that directory instead.

To reset everything:

- `promptx.json` for config
