...
enum DebugActions {
    private static let verboseDefaultsKey = "promptx.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "promptx.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = PromptXPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "PromptX", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = PromptXPaths.configURL
        guard
