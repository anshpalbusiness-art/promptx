...
struct PromptXMacCLI {
    static func main() async {
        ...
        default:
            fputs("promptx-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      promptx-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      promptx-mac discover [--timeout <ms>] [--json] [--include-local]
      promptx-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      promptx-mac connect
      promptx-mac connect --url ws://127.0.0.1:18789 --json
      promptx-mac discover --timeout 3000 --json
      promptx-mac wizard --mode local
    """)
}
