---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for PromptX deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, PromptX cron scheduling for periodic checks, or version status checks on a machine running PromptX (laptop, workstation, Pi, VPS).
---

# PromptX Host Hardening

## Overview

Assess and harden the host running PromptX, then align it to a user-defined risk tolerance without breaking access. Use PromptX security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim PromptX changes the host firewall, SSH, or OS updates; it does not.
...
5. PromptX gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if PromptX can access sensitive data.
...
### 2) Run PromptX security audits (read-only)

As part of the default read-only checks, run `promptx security audit --deep`. Only offer alternatives if the user requests them:

1. `promptx security audit` (faster, non-probing)
2. `promptx security audit --json` (structured output)

Offer to apply PromptX safe defaults (numbered):

1. `promptx security audit --fix`

Be explicit that `--fix` only tightens PromptX defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check PromptX version/update status (read-only)

As part of the default read-only checks, run `promptx update status`.
...
- Credential hygiene notes (location of PromptX creds, prefer disk encryption)
...
After PromptX install or first hardening pass, run at least one baseline audit and version check:

- `promptx security audit`
- `promptx security audit --deep`
- `promptx update status`

Ongoing monitoring is recommended. Use the PromptX cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `promptx cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `promptx update status`
...
Before creating, `promptx cron list` and match on exact `name`. If found, `promptx cron edit <id> ...`.
If not found, `promptx cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `promptx update status` (preferred for source checkouts and channels)
2. `npm view promptx version` (published npm version)

## PromptX command accuracy

Use only supported commands and flags:

- `promptx security audit [--deep] [--fix] [--json]`
- `promptx status` / `promptx status --deep`
- `promptx health --json`
- `promptx update status`
- `promptx cron add|list|runs|run`

Do not invent CLI flags or imply PromptX enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by PromptX compaction:
