...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "promptx.sandbox=1");
  args.push("--label", `promptx.sessionKey=${params.scopeKey}`);
  args.push("--label", `promptx.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `promptx.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("promptx.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`promptx sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`promptx sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("promptx sandbox recreate --all");
}
