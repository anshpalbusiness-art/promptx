...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# PromptX Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(promptx completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# PromptX Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(promptx completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $PROMPTX_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "promptx") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c promptx -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'promptx gateway' and not 'promptx other gateway', we need to check the sequence.
