...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolvePromptXAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("promptx models auth setup-token")}\` or \`${formatCliCommand("promptx configure")}\`.`
          : `Run \`${formatCliCommand("promptx configure")}\` or set an API key env var.`;
