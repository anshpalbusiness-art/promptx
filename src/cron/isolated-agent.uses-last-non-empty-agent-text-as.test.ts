...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "promptx-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".promptx", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<PromptXConfig> = {},
): PromptXConfig {
  const base: PromptXConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "promptx"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".promptx", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
