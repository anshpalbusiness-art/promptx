...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.PROMPTX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*promptx-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "promptx";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "promptx");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "promptx-gw");
  ...
  records.push(`_promptx-gw._tcp IN PTR ${instanceLabel}._promptx-gw._tcp`);
  records.push(`${instanceLabel}._promptx-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._promptx-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; promptx-content-hash: ${contentHash}\n${contentBody}`;
}
