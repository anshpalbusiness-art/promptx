...
import type { PromptXConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("promptx-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("promptx-bin-sherpa-");
      const modelDir = await makeTempDir("promptx-sherpa-model-");
      ...
      const cfg: PromptXConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("promptx-bin-whispercpp-");
      const modelDir = await makeTempDir("promptx-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("promptx-bin-gemini-");
      ...
      const cfg: PromptXConfig = { tools: { media: { image: {} } } };
