...
export function isCommandEnabled(cfg: PromptXConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: PromptXConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as PromptXConfig),
...
export function getCommandDetection(_cfg?: PromptXConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: PromptXConfig) {
