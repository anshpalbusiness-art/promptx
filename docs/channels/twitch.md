...
read_when:
  - Setting up Twitch chat integration for PromptX
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. PromptX connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
promptx plugins install @promptx/twitch
```

Local checkout (when running from a git repo):

```bash
promptx plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `PROMPTX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "promptx", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use PROMPTX_TWITCH_ACCESS_TOKEN env var)
...
```bash
PROMPTX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
