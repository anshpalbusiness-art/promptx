...
2. **Appends to log file** - Writes a JSON line to `~/.promptx/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.promptx/logs/commands.log ~/.promptx/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/promptx`:
   ```
   /home/username/.promptx/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
