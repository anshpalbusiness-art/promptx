...
describe("canvas host state dir defaults", () => {
  ...
  it("uses PROMPTX_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("PromptX Canvas");
    } finally {
      await handler.close();
