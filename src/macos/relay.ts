...
declare const __PROMPTX_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __PROMPTX_VERSION__ === "string" && __PROMPTX_VERSION__) ||
  process.env.PROMPTX_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensurePromptXCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[promptx] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[promptx] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
