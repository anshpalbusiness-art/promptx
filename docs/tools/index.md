---
summary: "Agent tool surface for PromptX (browser, canvas, nodes, message, cron) replacing legacy `promptx-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `promptx-*` skills
title: "Tools"
---

# Tools (PromptX)

PromptX exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `promptx-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `promptx.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, PromptX logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`promptx node run`).
...
- Requires a Brave API key (recommended: `promptx configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated PromptX-managed browser.
...
- Quick smoke: `promptx nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `promptx gateway` restart in-place)
