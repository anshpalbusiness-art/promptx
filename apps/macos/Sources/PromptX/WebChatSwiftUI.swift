...
private let webChatSwiftLogger = Logger(subsystem: "ai.promptx", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: PromptXChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PromptXChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(PromptXChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> PromptXChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                PromptXGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<PromptXChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any PromptXChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = PromptXChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: PromptXChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<PromptXChatView>) -> NSViewController
    {
