...
`;
    ...
    expect(parsed.promptx.emoji).toBe("💾");
    expect(parsed.promptx.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.promptx.emoji).toBe("📝");
    expect(parsed.promptx.events).toEqual(["command"]);
    expect(parsed.promptx.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.promptx.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"promptx": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"promptx": {"events": ["test"]}}');
  });
...
describe("resolvePromptXMetadata", () => {
  it("extracts promptx metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolvePromptXMetadata(frontmatter);
  ...
  it("returns undefined when promptx key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        promptx: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with PromptX" },
            { id: "npm", kind: "npm", package: "@promptx/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@promptx/hook");
  });
...
`;
    ...
    const promptx = resolvePromptXMetadata(frontmatter);
    expect(promptx).toBeDefined();
    expect(promptx?.emoji).toBe("💾");
    expect(promptx?.events).toEqual(["command:new"]);
    expect(promptx?.requires?.config).toEqual(["workspace.dir"]);
    expect(promptx?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const promptx = resolvePromptXMetadata(frontmatter);
    expect(promptx?.emoji).toBe("disk");
    expect(promptx?.events).toEqual(["command:new"]);
  });
});
