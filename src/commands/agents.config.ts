...
export function listAgentEntries(cfg: PromptXConfig): AgentEntry[] {
...
function resolveAgentName(cfg: PromptXConfig, agentId: string) {
...
function resolveAgentModel(cfg: PromptXConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PromptXConfig): AgentSummary[] {
