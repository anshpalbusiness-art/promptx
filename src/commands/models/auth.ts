...
import { readConfigFileSnapshot, type PromptXConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: PromptXConfig, model: string): PromptXConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("promptx plugins install")}\`.`,
    );
  }
