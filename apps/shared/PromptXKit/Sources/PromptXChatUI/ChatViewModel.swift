...
private let chatUILogger = Logger(subsystem: "ai.promptx", category: "PromptXChatUI")

@MainActor
@Observable
public final class PromptXChatViewModel {
    public private(set) var messages: [PromptXChatMessage] = []
    ...
    public var attachments: [PromptXPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [PromptXChatPendingToolCall] = []
    public private(set) var sessions: [PromptXChatSessionEntry] = []
    private let transport: any PromptXChatTransport
    ...
    private var pendingToolCallsById: [String: PromptXChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any PromptXChatTransport) {
    ...
    public var sessionChoices: [PromptXChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [PromptXChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [PromptXChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: PromptXPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [PromptXChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: PromptXChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [PromptXChatMessage]) -> [PromptXChatMessage] {
        var result: [PromptXChatMessage] = []
    ...
    private static func dedupeKey(for message: PromptXChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [PromptXChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> PromptXChatAttachmentPayload in
            PromptXChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> PromptXChatSessionEntry {
        PromptXChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: PromptXChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: PromptXChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: PromptXAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = PromptXChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> PromptXPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
