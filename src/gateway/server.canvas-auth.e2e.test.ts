...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.PROMPTX_CONFIG_PATH;
  const prevDisableCache = process.env.PROMPTX_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "promptx-canvas-auth-test-"));
  const configPath = path.join(dir, "promptx.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.PROMPTX_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.PROMPTX_DISABLE_CONFIG_CACHE;
    } else {
      process.env.PROMPTX_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
