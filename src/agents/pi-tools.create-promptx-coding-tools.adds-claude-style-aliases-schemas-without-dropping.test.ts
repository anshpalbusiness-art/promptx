...
import { __testing, createPromptXCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";
import { createPromptXTools } from "./promptx-tools.js";
...
describe("createPromptXCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createPromptXTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createPromptXCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "promptx-outside.txt");
