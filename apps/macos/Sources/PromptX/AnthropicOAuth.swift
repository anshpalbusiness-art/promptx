...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (PromptX token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: PromptXOAuthStore.AnthropicOAuthStatus = PromptXOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.promptx", category: "anthropic-oauth")
...
enum PromptXOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let promptxOAuthDirEnv = "PROMPTX_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "PromptX OAuth token file not found"
            case .unreadableFile: "PromptX OAuth token file not readable"
            case .invalidJSON: "PromptX OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in PromptX OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "PromptX OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.promptxOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".promptx", isDirectory: true)
