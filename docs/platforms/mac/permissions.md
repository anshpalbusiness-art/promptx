...
- Same path: run the app from a fixed location (for PromptX, `dist/PromptX.app`).
...
Workaround: move files into the PromptX workspace (`~/.promptx/workspace`) if you want to avoid per-folder grants.
