...
function resolveMode(cfg: PromptXConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("PromptX doctor");

  const root = await resolvePromptXPackageRoot({
  ...
  let cfg: PromptXConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("promptx configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("promptx config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("promptx setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("promptx doctor --fix")}" to apply changes.`);
  }
