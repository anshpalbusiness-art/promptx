...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: PromptXConfig;
  nextConfig: PromptXConfig;
  workspaceDir: string;
...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.promptx/promptx.json (gateway.auth.token) or PROMPTX_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("promptx config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("promptx doctor --generate-gateway-token")}`,
        "Web UI stores a copy in this browser's localStorage (promptx.control.settings.v1).",
        `Open the dashboard anytime: ${formatCliCommand("promptx dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control PromptX."
            : "Copy/paste this URL in a browser on this machine to control PromptX.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("promptx dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.promptx.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.promptx.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          "",
          "PromptX uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("promptx configure --section web")}`,
  ...
  await prompter.note(
    'What now: https://promptx.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control PromptX."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control PromptX.",
  );
