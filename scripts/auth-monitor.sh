...
#
# Suggested cron: */30 * * * * /home/admin/promptx/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send PromptX notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., promptx-alerts)
...
STATE_FILE="$HOME/.promptx/auth-monitor-state"
...
send_notification() {
    ...
    # Send via PromptX if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use promptx
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via PromptX to $NOTIFY_PHONE..."
            promptx send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! PromptX is down. Run: ssh l36 '~/promptx/scripts/mobile-reauth.sh'" "urgent"
    exit 1
