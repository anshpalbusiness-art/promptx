...
const ETC_PROMPTX_DIR = path.join(ROOT_DIR, "etc", "promptx");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "promptx.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcPromptXPath(...parts: string[]) {
  return path.join(ETC_PROMPTX_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcPromptXPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "promptx.json"))).toEqual({
      shared: true,
    });
  });
});
