...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `promptx sandbox explain`.
...
Legacy `agent.*` configs are migrated by `promptx doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=promptx-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${PROMPTX_STATE_DIR:-$HOME/.promptx}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
