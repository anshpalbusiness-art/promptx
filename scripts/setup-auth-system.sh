#!/bin/bash
# Setup PromptX Auth Management System
...
echo "=== PromptX Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'promptx-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  PromptX message: Send warning via PromptX itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/promptx-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/promptx-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/promptx-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now promptx-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status promptx-auth-monitor.timer"
echo ""
echo "Quick commands:"
