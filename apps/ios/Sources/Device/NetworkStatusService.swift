...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> PromptXNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> PromptXNetworkStatusPayload {
        let status: PromptXNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> PromptXNetworkStatusPayload {
        PromptXNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
