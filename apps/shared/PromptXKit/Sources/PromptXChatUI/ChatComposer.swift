...
@MainActor
struct PromptXChatComposer: View {
    @Bindable var viewModel: PromptXChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(PromptXChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(PromptXChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \PromptXPendingAttachment.id)
                { (att: PromptXPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            PromptXPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PromptXChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(PromptXChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(PromptXChatTheme.subtleCard)
        .clipShape(Capsule())
    }
