...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<PromptXTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = PromptXTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> PromptXTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return PromptXTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> PromptXTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> PromptXTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return PromptXTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> PromptXTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = PromptXTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: PromptXTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: PromptXAgentEventPayload.self) else {
                continue
            }
