...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.PROMPTX_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "promptx");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.PROMPTX_STATE_DIR = stateDir;
    process.env.PROMPTX_CONFIG_PATH = path.join(stateDir, "promptx.json");
