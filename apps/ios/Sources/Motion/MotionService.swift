...
final class MotionService: MotionServicing {
    func activities(params: PromptXMotionActivityParams) async throws -> PromptXMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[PromptXMotionActivityEntry], Error>) in
        ...
        return PromptXMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: PromptXPedometerParams) async throws -> PromptXPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<PromptXPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = PromptXPedometerPayload(
