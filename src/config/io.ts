...
import type { PromptXConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { comparePromptXVersions } from "./version.js";
...
function coerceConfig(value: unknown): PromptXConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as PromptXConfig;
}
...
function stampConfigVersion(cfg: PromptXConfig): PromptXConfig {
...
function warnIfConfigFromFuture(cfg: PromptXConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = comparePromptXVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer PromptX (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: PromptXConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PromptXConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as PromptXConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PromptXConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PromptXConfig) {
    clearConfigCache();
...
// module scope. `PROMPTX_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.PROMPTX_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.PROMPTX_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): PromptXConfig {
...
export async function writeConfigFile(cfg: PromptXConfig): Promise<void> {
  clearConfigCache();
