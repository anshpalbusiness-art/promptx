...
function setSlackDmPolicy(cfg: PromptXConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "PromptX";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for PromptX`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: PromptXConfig, allowFrom: string[]): PromptXConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
