...
import { resolvePromptXPackageRoot, resolvePromptXPackageRootSync } from "./promptx-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolvePromptXPackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "promptx" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolvePromptXPackageRootSync({
    argv1,
