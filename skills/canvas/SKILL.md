...
Display HTML content on connected PromptX nodes (Mac app, iOS, Android).
...
```
http://<tailscale-hostname>:18793/__promptx__/canvas/<file>.html
```
...
```bash
cat ~/.promptx/promptx.json | jq '.gateway.bind'
```

Then construct the URL:

- **loopback**: `http://127.0.0.1:18793/__openclaw__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__promptx__/canvas/<file>.html`
...
1. Check server bind: `cat ~/.promptx/promptx.json | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__promptx__/canvas/<file>.html`
...
Node is offline. Use `promptx nodes list` to find online nodes.
...
The canvas host serves from `/__promptx__/canvas/` prefix:

```
http://<host>:18793/__promptx__/canvas/index.html  → ~/clawd/canvas/index.html
http://<host>:18793/__promptx__/canvas/games/snake.html → ~/clawd/canvas/games/snake.html
```

The `/__promptx__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
