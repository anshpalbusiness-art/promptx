...
import type { PromptXConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: PromptXConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: PromptXConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = PromptXSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as PromptXConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as PromptXConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as PromptXConfig)),
    ),
  };
}
