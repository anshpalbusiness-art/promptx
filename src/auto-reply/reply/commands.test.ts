...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-commands-"));
...
function buildParams(commandBody: string, cfg: PromptXConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
