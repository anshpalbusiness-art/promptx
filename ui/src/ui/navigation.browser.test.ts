...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/promptx/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/promptx");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/promptx/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PROMPTX_CONTROL_UI_BASE_PATH__ = "/promptx";
    const app = mountApp("/promptx/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/promptx");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/promptx/sessions");
  });
