import type { PluginRuntime } from "promptx/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.PROMPTX_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-nostr-"));
  process.env.PROMPTX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.PROMPTX_STATE_DIR?.trim() || env.PROMPTX_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".promptx");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.PROMPTX_STATE_DIR = previous;
    }
