 ...
 * `promptx update`. Use it to verify the completion installation flow
...
\`promptx update\`, \`promptx doctor\`, and \`promptx onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# PromptX Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`promptx completion --install\` later to enable.`));
    return;
  }
