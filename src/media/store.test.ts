...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "PROMPTX_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.PROMPTX_STATE_DIR = path.join(home, ".promptx");
    ...
    await fs.mkdir(path.join(home, ".promptx"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.promptx${path.sep}media`);
