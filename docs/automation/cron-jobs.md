...
- Jobs persist under `~/.promptx/cron/` so restarts don’t lose schedules.
...
promptx cron list
promptx cron run <job-id>
promptx cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.promptx/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `promptx cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, PromptX defaults to `announce`.
...
- Job store: `~/.promptx/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.promptx/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
promptx cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
promptx cron edit <jobId> --agent ops
promptx cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
promptx cron run <jobId>
promptx cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
promptx cron edit <jobId> \
...
```bash
promptx system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`promptx system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `PROMPTX_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- PromptX applies exponential retry backoff for recurring jobs after consecutive errors:
