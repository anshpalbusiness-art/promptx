...
import { resolvePromptXPackageRoot } from "../infra/promptx-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "promptx doctor entry": "Checking doctor entrypoint",
  "promptx doctor": "Running doctor checks",
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("promptx@")) {
    return trimmed.slice("promptx@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "promptx.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("promptx completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.PROMPTX_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", PROMPTX_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `PROMPTX_GIT_DIR points at a non-git directory: ${params.dir}. Set PROMPTX_GIT_DIR to an empty folder or an promptx checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`PROMPTX_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolvePromptXPackageRoot({
  ...
  defaultRuntime.log(theme.heading("PromptX update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating PromptX..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this PromptX install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("promptx doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("promptx gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g promptx@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g promptx@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.PROMPTX_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.PROMPTX_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("promptx gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("promptx doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("promptx gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("promptx gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `promptx update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `PROMPTX_GIT_DIR points at a non-git directory: ${gitDir}. Set PROMPTX_GIT_DIR to an empty folder or an promptx checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via PROMPTX_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update PromptX to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["promptx update", "Update a source checkout (git)"],
        ["promptx update --channel beta", "Switch to beta channel (git + npm)"],
        ["promptx update --channel dev", "Switch to dev channel (git + npm)"],
        ["promptx update --tag beta", "One-off update to a dist-tag or version"],
        ["promptx update --no-restart", "Update without restarting the service"],
        ["promptx update --json", "Output result as JSON"],
        ["promptx update --yes", "Non-interactive (accept downgrade prompts)"],
        ["promptx update wizard", "Interactive update wizard"],
        ["promptx --update", "Shorthand for promptx update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run promptx update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.promptx.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["promptx update status", "Show channel + version status."],
          ["promptx update status --json", "JSON output."],
          ["promptx update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.promptx.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
