...
This is an **alpha** iOS app that connects to an PromptX Gateway as a `role: node`.
...
3. On iOS: PromptX → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project PromptX.xcodeproj -scheme PromptX -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/PromptXKit` contains the shared transport/types used by the iOS app.
