...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.promptx", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: PromptXPeekabooBridgeServices?
    private static var promptxSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("PromptX", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = PromptXPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.promptxSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.promptxSocketPath, privacy: .public)")
    }
...
@MainActor
private final class PromptXPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.promptx.peekaboo")
