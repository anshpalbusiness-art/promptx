...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.PROMPTX_STATE_DIR;
  previousConfigPath = process.env.PROMPTX_CONFIG_PATH;
  previousSkipBrowserControl = process.env.PROMPTX_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.PROMPTX_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.PROMPTX_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.PROMPTX_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "promptx-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.PROMPTX_STATE_DIR = path.join(tempHome, ".promptx");
  delete process.env.PROMPTX_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.PROMPTX_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.PROMPTX_SKIP_GMAIL_WATCHER = "1";
  process.env.PROMPTX_SKIP_CANVAS_HOST = "1";
  process.env.PROMPTX_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "promptx-test-no-bundled-extensions")
    : "promptx-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "promptx-test-"))
    : path.join(tempHome, ".promptx-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.PROMPTX_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.PROMPTX_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.PROMPTX_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.PROMPTX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.PROMPTX_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.PROMPTX_SKIP_CANVAS_HOST;
    } else {
      process.env.PROMPTX_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.PROMPTX_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.PROMPTX_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.PROMPTX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.PROMPTX_GATEWAY_TOKEN = fallbackToken;
  }
