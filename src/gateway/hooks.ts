...
export function resolveHooksConfig(cfg: PromptXConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-promptx-token"] === "string" ? req.headers["x-promptx-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
