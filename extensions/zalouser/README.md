...
PromptX extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
promptx channels login --channel zalouser
...
```bash
promptx message send --channel zalouser --target <threadId> --message "Hello from PromptX!"
```
...
```bash
promptx channels login --channel zalouser              # Login via QR
promptx channels login --channel zalouser --account work
promptx channels status --probe
promptx channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
promptx directory self --channel zalouser
promptx directory peers list --channel zalouser --query "name"
promptx directory groups list --channel zalouser --query "work"
promptx directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
promptx message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
promptx message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `promptx channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
promptx channels login --channel zalouser --account work
promptx message send --channel zalouser --account work --target <id> --message "Hello"
