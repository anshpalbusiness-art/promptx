...
enum CommandResolver {
    private static let projectRootDefaultsKey = "promptx.gatewayProjectRootPath"
    private static let helperName = "promptx"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let promptxEntry = root.appendingPathComponent("promptx.mjs").path
        if FileManager().isReadableFile(atPath: promptxEntry) { return promptxEntry }
        let binEntry = root.appendingPathComponent("bin/promptx.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/promptx")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let promptxPaths = self.promptxManagedPaths(home: home)
        if !promptxPaths.isEmpty {
            extras.insert(contentsOf: promptxPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + promptxPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func promptxManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".promptx"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func promptxExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectPromptXExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("promptx.mjs").path,
            root.appendingPathComponent("bin/promptx.js").path,
        ]
    ...
    static func hasAnyPromptXInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.promptxExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func promptxNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let promptxPath = self.projectPromptXExecutable(projectRoot: root) {
                return [promptxPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "promptx", subcommand] + extraArgs
            }
            if let promptxPath = self.promptxExecutable(searchPaths: searchPaths) {
                return [promptxPath, subcommand] + extraArgs
            }

            let missingEntry = """
            promptx entrypoint missing (looked for dist/index.js or promptx.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real promptx CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/promptx"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/promptx.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/promptx.mjs"
            node "$PRJ/promptx.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/promptx.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/promptx.js"
            node "$PRJ/bin/promptx.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent promptx"
          pnpm --silent promptx \(quotedArgs);
        else
          echo "promptx CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? PromptXConfigFile.loadDict()
