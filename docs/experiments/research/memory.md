...
read_when:
  - Designing workspace memory (~/.promptx/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep PromptX integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.promptx/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.promptx/workspace` as canonical human-readable memory.
...
```
~/.promptx/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in PromptX**, but keep a separable core library.

### Why integrate into PromptX?

- PromptX already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `promptx memory recall "…" --k 25 --since 30d`
  - `promptx memory reflect --since 7d`
...
Pragmatic take for `~/.promptx/workspace`:
