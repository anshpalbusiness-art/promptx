---
summary: "Use Synthetic's Anthropic-compatible API in PromptX"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. PromptX registers it as the
...
Note: PromptX's Anthropic client appends `/v1` to the base URL, so use
