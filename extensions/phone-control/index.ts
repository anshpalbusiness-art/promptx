import type { PromptXPluginApi, PromptXPluginService } from "promptx/plugin-sdk";
...
function normalizeDenyList(cfg: PromptXPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: PromptXPluginApi["config"]): string[] {
...
export default function register(api: PromptXPluginApi) {
  ...
  const timerService: PromptXPluginService = {
