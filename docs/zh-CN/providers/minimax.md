---
read_when:
  - 你想在 PromptX 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 PromptX 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
promptx plugins enable minimax-portal-auth  # 如果已加载则跳过
promptx gateway restart  # 如果 Gateway 网关已在运行则重启
...
- 使用 `promptx models list` 和 `promptx models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `promptx configure` 并选择 **MiniMax M2.1**，或
