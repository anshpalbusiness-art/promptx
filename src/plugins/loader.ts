...
import { discoverPromptXPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PromptXPluginDefinition;
  register?: PromptXPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PromptXPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PromptXPluginDefinition;
...
export function loadPromptXPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverPromptXPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "promptx/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: PromptXPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as PromptXPluginModule;
    } catch (err) {
