...
- Paths: add `PROMPTX_HOME` for overriding the home directory used by internal path resolution. (#12091) Thanks @sebslight.
...
- Paths: structurally resolve `PROMPTX_HOME`-derived home paths and fix Windows drive-letter handling in tool meta shortening. (#12125) Thanks @mcaxtr.
...
- State dir: honor `PROMPTX_STATE_DIR` for default device identity and canvas storage paths. (#4824) Thanks @kossoy.
...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (promptx-cn).
...
- Docs: finish renaming the QMD memory docs to reference the PromptX state dir.
...
- Skills: update session-logs paths from .clawdbot to .promptx. (#4502)
...
- Build: align npm tar override and bin metadata so the `promptx` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update PromptX branding + install links.
...
- Rebrand: rename the npm package/CLI to `promptx`, add a `promptx` compatibility shim, and move extensions to the `@promptx/*` scope.
...
- macOS: finish PromptX app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Skills: update session-logs paths to use ~/.promptx. (#4502) Thanks @bonald.
...
- CLI: restart the gateway by default after `promptx update`; add `--no-restart` to skip it. (commit 2c85b1b40)
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `promptx models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- CLI: flatten node service commands under `promptx node` and remove `service node` docs.
- CLI: move gateway service commands under `promptx gateway` and add `gateway probe` for reachability.
...
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `promptx update`.
- Plugins: share npm plugin update logic between `promptx update` and `promptx plugins update`.
...
- Build: use tsgo for dev/watch builds by default (opt out with `PROMPTX_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `promptx doctor --fix` to repair, then update plugins (`promptx plugins update`) if you use any.
...
- CLI: keep `promptx logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `promptx memory status`.
...
- Memory: probe sqlite-vec availability in `promptx memory status`.
...
- **BREAKING:** `promptx message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
...
- **BREAKING:** `promptx plugins install <path>` now copies into `~/.promptx/extensions` (use `--link` to keep path-based loading).
...
- Directory: unify `promptx directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `promptx.hooks` manifests and `promptx hooks install/update`.
...
- CLI: speed up `promptx sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `promptx models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@promptx/msteams` via `promptx plugins install @promptx/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `promptx-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `promptx security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `promptx models status`, and update docs.
- CLI: add `--json` output for `promptx daemon` lifecycle/install commands.
...
- Fix: make `promptx update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `promptx configure`, even if no other sections are chosen.
...
- Security: expanded `promptx security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `promptx security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `promptx browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.promptx-dev`).
...
- Memory: new `promptx memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.promptx/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `promptx dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `promptx doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `promptx doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `promptx plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `promptx.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- CLI: `promptx status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `promptx update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `promptx reset` and `promptx uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `promptx gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `promptx status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `promptx sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `promptx sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “PromptX” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `promptx update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/promptx`. (#660)
...
- CLI: `promptx message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `promptx models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.promptx/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `promptx pairing list <provider>` + `promptx pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `promptx logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$PROMPTX_STATE_DIR/.env` (`~/.promptx/.env`) as a fallback after CWD `.env`.
...
- CLI: run `promptx agent` via the Gateway by default; use `--local` to force embedded mode.
