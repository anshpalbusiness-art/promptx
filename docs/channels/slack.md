...
3. Set tokens for PromptX and start the gateway.
...
7. Slash Commands → create `/promptx` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
PromptX can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, PromptX falls back to the user token.
...
```json
{
  "display_information": {
    "name": "PromptX",
    "description": "Slack connector for PromptX"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/promptx",
        "description": "Send a message to PromptX",
        "should_escape": false
      }
    ]
  },
...
By default, PromptX replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, PromptX infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `promptx pairing approve slack <code>`.
...
- On startup, PromptX resolves channel/user names in allowlists to IDs (when tokens allow)
