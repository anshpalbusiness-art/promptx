...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".promptx");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.PROMPTX_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$PROMPTX_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via PROMPTX_STATE_DIR.
