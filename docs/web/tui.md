...
```bash
promptx tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `promptx tui --deliver`
...
- Check the Gateway logs: `promptx logs --follow`.
- Confirm the agent can run: `promptx status` and `promptx models status`.
...
- No agents in picker: check `promptx agents list` and your routing config.
