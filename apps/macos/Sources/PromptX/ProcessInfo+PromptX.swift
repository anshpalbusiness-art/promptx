...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.promptx.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["PROMPTX_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "promptx.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "promptx.nixMode") { return true }
