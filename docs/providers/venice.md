---
summary: "Use Venice AI privacy-focused models in PromptX"
read_when:
  - You want privacy-focused inference in PromptX
...
```bash
promptx onboard --auth-choice venice-api-key
```
...
```bash
promptx chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, PromptX shows all available Venice models. Pick based on your needs:
...
```bash
promptx models set venice/claude-opus-45
promptx models set venice/llama-3.3-70b
```

List all available models:

```bash
promptx models list | grep venice
```

## Configure via `promptx configure`

1. Run `promptx configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
PromptX automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
promptx chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
promptx chat --model venice/claude-opus-45

# Use uncensored model
promptx chat --model venice/venice-uncensored

# Use vision model with image
promptx chat --model venice/qwen3-vl-235b-a22b

# Use coding model
promptx chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `promptx models list` to see currently available models. Some models may be temporarily offline.
