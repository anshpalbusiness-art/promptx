...
const SEARCH_TOOL = "https://docs.promptx.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.promptx.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('promptx docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.promptx.ai/");
      runtime.log(`Search: ${formatCliCommand('promptx docs "your query"')}`);
    }
    return;
  }
