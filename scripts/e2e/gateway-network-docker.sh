...
IMAGE_NAME="promptx-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="promptx-net-e2e-$$"
GW_NAME="promptx-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "PROMPTX_GATEWAY_TOKEN=$TOKEN" \
	  -e "PROMPTX_SKIP_CHANNELS=1" \
	  -e "PROMPTX_SKIP_GMAIL_WATCHER=1" \
	  -e "PROMPTX_SKIP_CRON=1" \
	  -e "PROMPTX_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
